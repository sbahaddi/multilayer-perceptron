...
class Data:
    ...
    def read_data(self, file):
        try:
            data = pd.read_csv(file, header=None, usecols=[
    ...
    def init_data(self):
        ...
        self.X_train, self.y_train = train[:, 1:].astype(
    ...
    def split_data(self):
        ...
        categories = [self.raw_data[self.raw_data[:, 0] == l]
                      for l in cfg.categories]

        ratios = [int(len(cat) * cfg.split_val // 100) for cat in categories]

        train = np.concatenate([cat[: ratios[i]]
                               for i, cat in enumerate(categories)])
        np.random.shuffle(train)

        val = np.concatenate([cat[ratios[i]:]
                             for i, cat in enumerate(categories)])
